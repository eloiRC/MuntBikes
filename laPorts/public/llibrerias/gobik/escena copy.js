/** Modificación: 03/11/2019 Modificación: Detector de compatibilidad WebGL intregrado en escena enlace. **/
...
/** Modificación: 03/11/2019 Modificación: SceneUtils intregrado en el script de la escena para enlaces. **/
...
// Genera la escena de la aplicación
...
/** Modificación: 01/10/2020 Modificación: Añadido giro de la escena al terminar la carga, y cada 20 segundos sin interactuación del usuario. Se aplica solo en los enlaces. **/
	...
	/** Modificación: 14/10/2020 Modificación: Eliminado giro escena de los enlaces públicos, pendiente de modificar comportamiento. **/
	
		// Rotación automática
		//if (rotacion_iniciada===false) rotacion_Inicio(scene,camera,renderer);
		
	/** Fin Modificación 14/10/2020 **/ 
	}
...
/** Modificación: 03/12/2019 Modificación: Optimizaciones al crear escena enlace. **/
...
function encuadreEscena(arrayObjetos,escenaOriginal,w,h,cam=null)
	...
	let escena = escenaOriginal.clone();
	
	let n = 0;
	let padre = null;
	
	// genera un bounding box del objeto padre
	BB = new THREE.Box3().setFromObject(escena);
	...
	if (dentro)
		...
		// Aleja un poco después del encuadre (Para Gobik)
		camaraTemporal.translateZ(50.0);
		camaraTemporal.updateProjectionMatrix();
		pasos += 100;
		...
		// Aleja un poco después del encuadre (Para Gobik)
		camaraTemporal.translateZ(100.0);
		camaraTemporal.updateProjectionMatrix();
		pasos += 100;
	...
	vertex.forEach(function(obj) { scene.remove(obj) });
	geometry = null;
	delete geometry;
	material = null;
	delete material;
