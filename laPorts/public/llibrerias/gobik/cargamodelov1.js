...
function nuevoModeloEscena(propiedades,renderizar=false, callback=null)
	...
	// carga el modelo
		loader.load( 'escena/modelos/'+propiedades.modelo, function (geometry, materials) {
			...
			/** Modificación: 09/12/2019 Modificación: Liberación de memoria después de cargar cada objeto y sus mapas en enlace. **/
...
let objetos_a_cargar = [ { nombre: "", mapas: 0, mapasCargados: 0, objetoCargado: false} ];
...
function montarEscena(propiedades)
	...
	if (num>0)
		...
		numero_Propiedades = num;
		for (let i=0;i<num;i++)
			{
			if (i>=num)
				nuevoModeloEscena(propiedades[i],true);
			else nuevoModeloEscena(propiedades[i],false);
			}
		}
	...
	idControlCarga = setInterval(function(){ 
		...
		// obtiene todos los objetos en la escena que no sean luces
		let objetosEscena = 0;
		scene.children.forEach(function(obj) {
			let nombre = obj.name.toUpperCase();

			if ((nombre.indexOf("LUZ-")<0)&&(nombre.indexOf("AXES")<0)) objetosEscena++;
			});

		// controla la carga de objetos
		let num = propiedades.length;
		console.log("objetos cargados:",num);
		
		console.log("num:",(num),"objetosEscena:",objetosEscena,"numero_Propiedades:",numero_Propiedades,"controlCarga:",controlCarga);
		
		if ((((num)===objetosEscena)&&(numero_Propiedades===controlCarga))||(numero_Propiedades===0))
			{
